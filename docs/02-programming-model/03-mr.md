...
你的程序使用的是**虚拟地址（VA）**，但网卡 DMA 需要的是**物理地址（PA）**。更麻烦的是：
...
| `IBV_ACCESS_REMOTE_WRITE` | 远端写权限 | 对方要 RDMA WRITE 到你的内存 |
| `IBV_ACCESS_REMOTE_READ` | 远端读权限 | 对方要 RDMA READ 你的内存 |
...
`ibv_reg_mr` 成功后，你会得到一个 `ibv_mr` 结构：
...
!!! note "rkey 是安全凭证，不是地址"
    远端地址本身不是权限——有了 `remote_addr` 没用，你必须有正确的 `remote_rkey` 才能访问对方的内存。这就是 RDMA 安全模型的基石：你可以随意公布你的内存地址，但只要不公布 rkey，别人就无法访问。
...
Memory Registration 的开销可能远超你的想象。在性能敏感的场景下，必须注意两点。
...
!!! note "大页的权衡"
    大页的缺点是内存粒度更大——如果你只需要 4KB，却要占用 2MB。但对于绝大多数 RDMA 应用，这不是问题（你的数据量通常远大于 2MB）。
...
!!! note "MW 是高级特性"
    大多数程序不需要 MW。只有当你需要频繁改变远端访问权限时才考虑使用。简单应用用 MR 就足够了。
