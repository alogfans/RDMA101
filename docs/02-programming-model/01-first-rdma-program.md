...
!!! note "为什么要这么多对象？"
    RDMA 的设计哲学是"显式声明"：你需要什么资源、什么权限、什么访问模式，都要明确告诉网卡。这种显式性是 RDMA 安全性和性能的基础。后续章节会详细讲解每个对象的作用和配置方法。
...
!!! note "rkey 的安全意义"
    `rkey` 不是简单的形式主义——它是 RDMA 安全模型的基石。远端没有正确的 `rkey`，就无法访问你的内存。这防止了任意 RDMA 请求随意读写你的内存。
...
RDMA 操作是异步的：你投递请求后，网卡在后台执行，你的程序可以做别的事情。那么怎么知道操作完成了？
