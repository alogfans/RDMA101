...
!!! note "CQ 溢出的后果"
    如果 CQ 满了，网卡无法写入新的 CQE，会产生异步错误事件。你的程序应该通过 `ibv_get_async_event` 监听这类事件。

## 2.5.3 Work Completion（WC）结构：网卡给你的"收条"

每个 CQE 对应一个 Work Completion（WC），包含一个已完成的 WR 的详细信息。你可以把 WC 理解为网卡的"收条"——告诉你"这个请求处理完了，结果是什么"。
